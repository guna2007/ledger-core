...
public class AccountController {
	...
	public ResponseEntity<ApiResponse<TransactionResponse>> deposit(@Valid @RequestBody DepositRequest request) {
		...
		if (!accountService.isAccountOwnedByUser(request.getAccountNumber(), userId)) {
			return ResponseEntity.status(403).body(ApiResponse.error("You don't have access to this account"));
		}

		TransactionResponse transaction = accountService.deposit(request, userId);
	...
	public ResponseEntity<ApiResponse<TransactionResponse>> withdraw(@Valid @RequestBody WithdrawRequest request) {
		...
		TransactionResponse transaction = accountService.withdraw(request, userId);
